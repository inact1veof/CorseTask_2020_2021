...
def main(date, device, user):
    ...
    resultStr = f"INSERT Forecasts VALUES(%s, '%s', %s, %s, %s, %s)" % ('1', dateTime.strftime("%d.%m.%Y %H:%M:%S"), value_id, real_value_id, user, rmse_value_id)
    ...
    RmseDates.clear()
    RmseValues.clear()
    print(f"Выполнен прогноз алгоритмом 1 для данных ")
